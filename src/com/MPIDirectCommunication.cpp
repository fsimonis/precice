...
void
MPIDirectCommunication::acceptConnection(std::string const &acceptorName,
...
void
MPIDirectCommunication::requestConnection(std::string const &acceptorName,
                                          std::string const &requesterName,
                                          int                requesterRank,
...
void
MPIDirectCommunication::reduceSum(int itemToSend, int &itemsToReceive)
{
  TRACE();
  int rank = -1;
  MPI_Comm_rank(_globalCommunicator, &rank);
...
void
MPIDirectCommunication::reduceSum(int itemToSend, int &itemsToReceive, int rankMaster)
{
  TRACE();
  // _comunicator did't work here as we seem to have two communicators, one with the master and one with the slaves
...
void
MPIDirectCommunication::broadcast(const int *itemsToSend, int size)
{
  TRACE(size);
...
void
MPIDirectCommunication::broadcast(const double *itemsToSend, int size)
{
  TRACE(size);
