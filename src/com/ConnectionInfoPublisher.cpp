...
std::string impl::hashedFilePath(const std::string &acceptorName, const std::string &requesterName, const std::string &meshName, int rank)
{
  ...
  std::string const            s    = acceptorName + meshName + requesterName + std::to_string(rank);
...
std::string ConnectionInfoPublisher::getFilename() const
{
  ...
  auto hashed = impl::hashedFilePath(acceptorName, requesterName, meshName, rank);
