...
int
ExportVTKXML::getType() const
{
  return constants::exportVTKXML();
}

void
ExportVTKXML::doExport(
...
void
ExportVTKXML::processDataNamesAndDimensions(
    mesh::Mesh &mesh)
{
  ...
  for (mesh::PtrData data : mesh.data()) { // Plot vertex data
    ...
    if (dataDimensions > 1) {
      ...
      for (size_t count = 0; count < mesh.vertices().size(); count++) {
        ...
        if (dataDimensions == 2) {
          outFile << "0.0"
                  << " "; //2D data needs to be 3D for vtk
        }
        outFile << " ";
      }
    }
