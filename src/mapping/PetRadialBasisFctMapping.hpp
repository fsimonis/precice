...
private:
  ...
  void
  printMappingInfo(int inputDataID, int dim) const;

  /// Toggles use of preallocation for matrix C and A
  const Preallocation _preallocation;

  void
  estimatePreallocationMatrixC(int rows, int cols, mesh::PtrMesh mesh);

  void
  estimatePreallocationMatrixA(int rows, int cols, mesh::PtrMesh mesh);

  void
  computePreallocationMatrixC(const mesh::PtrMesh inMesh);

  void
  computePreallocationMatrixA(const mesh::PtrMesh inMesh, const mesh::PtrMesh outMesh);

  std::vector<std::vector<std::pair<int, double>>>
  savedPreallocationMatrixC(const mesh::PtrMesh inMesh);

  std::vector<std::vector<std::pair<int, double>>>
  savedPreallocationMatrixA(const mesh::PtrMesh inMesh, const mesh::PtrMesh outMesh);

  std::vector<std::vector<std::pair<int, double>>>
  bgPreallocationMatrixC(const mesh::PtrMesh inMesh);

  std::vector<std::vector<std::pair<int, double>>>
  bgPreallocationMatrixA(const mesh::PtrMesh inMesh, const mesh::PtrMesh outMesh);
};
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::computeMapping()
{
  TRACE();
  ...
  ierr = ISCreateGeneral(utils::Parallel::getGlobalCommunicator(), myIndizes.size(), myIndizes.data(), PETSC_COPY_VALUES, &ISlocal);
  CHKERRV(ierr);
  ierr = ISSetPermutation(ISlocal);
  CHKERRV(ierr);
  ierr = ISInvertPermutation(ISlocal, myIndizes.size(), &ISlocalInv);
  CHKERRV(ierr);
  ierr = ISAllGather(ISlocalInv, &ISglobal);
  CHKERRV(ierr); // Gather the IS from all processors
  ...
  ierr = ISCreateStride(utils::Parallel::getGlobalCommunicator(), ownerRangeAEnd - ownerRangeABegin, ownerRangeABegin, 1, &ISidentity);
  CHKERRV(ierr);
  ierr = ISSetIdentity(ISidentity);
  CHKERRV(ierr);
  ierr = ISAllGather(ISidentity, &ISidentityGlobal);
  CHKERRV(ierr);
  ...
  ierr = ISCreateStride(PETSC_COMM_SELF, sepPolyparams, 0, 1, &ISpolyparams);
  CHKERRV(ierr);
  ierr = ISSetIdentity(ISpolyparams);
  CHKERRV(ierr);
  ierr = ISLocalToGlobalMappingCreateIS(ISpolyparams, &ISpolyparamsMapping);
  CHKERRV(ierr);

  // Sets the mappings on the matrices
  ierr = MatSetLocalToGlobalMapping(_matrixC, _ISmapping, _ISmapping);
  CHKERRV(ierr); // Set mapping for rows and cols
  ierr = MatSetLocalToGlobalMapping(_matrixA, ISidentityMapping, _ISmapping);
  CHKERRV(ierr); // Set mapping only for cols, use identity for rows
  ierr = MatSetLocalToGlobalMapping(_matrixQ, _ISmapping, ISpolyparamsMapping);
  CHKERRV(ierr);
  ierr = MatSetLocalToGlobalMapping(_matrixV, ISidentityMapping, ISpolyparamsMapping);
  CHKERRV(ierr);

  // Destroy all local index sets and mappings
  ierr = ISDestroy(&ISlocal);
  CHKERRV(ierr);
  ierr = ISDestroy(&ISlocalInv);
  CHKERRV(ierr);
  ierr = ISDestroy(&ISglobal);
  CHKERRV(ierr);
  ierr = ISDestroy(&ISidentity);
  CHKERRV(ierr);
  ierr = ISDestroy(&ISidentityGlobal);
  CHKERRV(ierr);
  ierr = ISLocalToGlobalMappingDestroy(&ISidentityMapping);
  CHKERRV(ierr);
  ierr = ISDestroy(&ISpolyparams);
  CHKERRV(ierr);
  ierr = ISLocalToGlobalMappingDestroy(&ISpolyparamsMapping);
  CHKERRV(ierr);
  ...
  for (const mesh::Vertex &inVertex : inMesh->vertices()) {
    ...
    if (_polynomial == Polynomial::ON or _polynomial == Polynomial::SEPARATE) {
      ...
      if (_polynomial == Polynomial::ON) {
        ierr = MatSetValuesLocal(_matrixC, colNum, colIdx, 1, &row, rowVals, INSERT_VALUES);
        CHKERRV(ierr);
      }
      else if (_polynomial == Polynomial::SEPARATE) {
        ierr = MatSetValuesLocal(_matrixQ, 1, &row, colNum, colIdx, rowVals, INSERT_VALUES);
        CHKERRV(ierr);
      }
      colNum = 0;
    }
    ...
    ierr = MatSetValuesLocal(_matrixC, 1, &row, colNum, colIdx, rowVals, INSERT_VALUES);
    CHKERRV(ierr);
  }
  ...
  for (int row = ownerRangeABegin; row < ownerRangeAEnd; row++) {
    ...
    if (_polynomial == Polynomial::ON or _polynomial == Polynomial::SEPARATE) {
      ...
      ierr = MatSetValuesLocal(m, 1, &row, colNum, colIdx, rowVals, INSERT_VALUES);
      CHKERRV(ierr);
      colNum = 0;
    }
    ...
    ierr = MatSetValuesLocal(_matrixA, 1, &row, colNum, colIdx, rowVals, INSERT_VALUES);
    CHKERRV(ierr);
  }
  ...
  if (useRescaling and (_polynomial == Polynomial::SEPARATE)) {
    ...
    ierr = MatCreateVecs(_matrixC, nullptr, &rescalingCoeffs.vector);
    CHKERRV(ierr);
    VecSet(rhs, 1);
    rhs.assemble();
...
template<typename RADIAL_BASIS_FUNCTION_T>
bool
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::hasComputedMapping() const
{
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::clear()
{
  _matrixC.reset();
  _matrixA.reset();
  _matrixQ.reset();
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::map(int inputDataID, int outputDataID)
{
  ...
  if (getConstraint() == CONSERVATIVE) {
    ...
    for (int dim = 0; dim < valueDim; dim++) {
      ...
      if (_polynomial == Polynomial::SEPARATE) {
        ...
        ierr = KSPSolveTranspose(_QRsolver, tau, sigma);
        CHKERRV(ierr);
    ...
    ierr = VecSetLocalToGlobalMapping(in, _ISmapping);
    CHKERRV(ierr);
    ...
    for (int dim = 0; dim < valueDim; dim++) {
      ...
      for (const auto &vertex : input()->vertices()) {
        ierr = VecSetValueLocal(in, vertex.getGlobalIndex() + polyparams, inValues[count * valueDim + dim], INSERT_VALUES);
        CHKERRV(ierr);
        count++;
      }
      in.assemble();
      ...
      if (useRescaling and (_polynomial == Polynomial::SEPARATE)) {
        ...
        PetscReal     min;
        ierr = MatMult(_matrixA, rescalingCoeffs, temp);
        CHKERRV(ierr); // get the output of g(x) = 1
        ierr = VecMin(temp, nullptr, &min);
        CHKERRV(ierr); // check for zeros before devision
        if (min < 1e-6)
          WARN("Zero elements found in rescaling, omit rescaling.");
        else
          ierr = VecPointwiseDivide(out, out, temp);
        CHKERRV(ierr);
      }
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::tagMeshFirstRound()
{
  TRACE();
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::tagMeshSecondRound()
{
  TRACE();
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::printMappingInfo(int inputDataID, int dim) const
{
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::estimatePreallocationMatrixC(
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::estimatePreallocationMatrixA(
    int rows, int cols, mesh::PtrMesh mesh)
{
  std::ignore = rows;
  std::ignore = cols;
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::computePreallocationMatrixC(const mesh::PtrMesh inMesh)
{
...
template<typename RADIAL_BASIS_FUNCTION_T>
void
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::computePreallocationMatrixA(
...
template<typename RADIAL_BASIS_FUNCTION_T>
std::vector<std::vector<std::pair<int, double>>>
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::savedPreallocationMatrixC(
...
template<typename RADIAL_BASIS_FUNCTION_T>
std::vector<std::vector<std::pair<int, double>>>
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::savedPreallocationMatrixA(
    const mesh::PtrMesh inMesh, const mesh::PtrMesh outMesh)
{
...
template<typename RADIAL_BASIS_FUNCTION_T>
std::vector<std::vector<std::pair<int, double>>>
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::bgPreallocationMatrixC(
    const mesh::PtrMesh inMesh)
{
...
template<typename RADIAL_BASIS_FUNCTION_T>
std::vector<std::vector<std::pair<int, double>>>
PetRadialBasisFctMapping<RADIAL_BASIS_FUNCTION_T>::bgPreallocationMatrixA(
    const mesh::PtrMesh inMesh, const mesh::PtrMesh outMesh)
{
