...
void
NearestNeighborMapping::computeMapping()
{
...
bool
NearestNeighborMapping::hasComputedMapping() const
{
...
void
NearestNeighborMapping::tagMeshFirstRound()
{
  TRACE();
  ...
  if (getConstraint() == CONSISTENT) {
    for (mesh::Vertex &v : input()->vertices()) {
      if (utils::contained(v.getID(), _vertexIndices))
        v.tag();
    }
  }
  else {
...
void
NearestNeighborMapping::tagMeshSecondRound()
{
  TRACE();
