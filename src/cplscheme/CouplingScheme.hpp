...
public:
  ...
  virtual void
  addComputedTime(double timeToAdd) = 0;
   ...
   */
  virtual bool
  willDataBeExchanged(double lastSolverTimestepLength) const = 0;
   ...
   */
  virtual double
  getNextTimestepMaxLength() const = 0;
  ...
  virtual bool
  isCouplingOngoing() const = 0;

  /// Returns true, when the accessor can advance to the next timestep.
  virtual bool
  isCouplingTimestepComplete() const = 0;

  /// Returns true, if the given action has to be performed by the accessor.
  virtual bool
  isActionRequired(const std::string &actionName) const = 0;

  /// Tells the coupling scheme that the accessor has performed the given action.
  virtual void
  performedAction(const std::string &actionName) = 0;

  /// Sets an action required to be performed by the accessor.
  virtual void
  requireAction(const std::string &actionName) = 0;

  /// Returns a string representation of the current coupling state.
  virtual std::string
  printCouplingState() const = 0;
