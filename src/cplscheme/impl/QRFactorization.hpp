...
public:
  ...
  /**
    ...
    */
  bool
  insertColumn(int k, const Eigen::VectorXd &v, double singularityLimit = 0);
  ...
  /**
    ...
    */
  void
  pushFront(const Eigen::VectorXd &v);
  ...
  /**
    ...
    */
  void
  pushBack(const Eigen::VectorXd &v);
  ...
  /**
    ...
    */
  void
  applyFilter(double singularityLimit, std::vector<int> &delIndices, Eigen::MatrixXd &V);
...
private:
  ...
  */
  int
  orthogonalize_stable(Eigen::VectorXd &v, Eigen::VectorXd &r, double &rho, int colNum);
  ...
  *   if ||v_orth||/||v|| approx 0, no unit vector is inserted.
   */
  int
  orthogonalize(Eigen::VectorXd &v, Eigen::VectorXd &r, double &rho, int colNum);

  /**
  * @short computes parameters for givens matrix G for which  (x,y)G = (z,0). replaces (x,y) by (z,0)
  */
  void
  computeReflector(givensRot &grot, double &x, double &y);
  ...
  */
  void
  applyReflector(const givensRot &grot, int k, int l, Eigen::VectorXd &p, Eigen::VectorXd &q);
