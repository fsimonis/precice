...
void
openViewer(PetscViewer &viewer, std::string filename, VIEWERFORMAT format, MPI_Comm comm)
{
...
Vector::Vector(Mat &m, std::string name, LEFTRIGHT type)
{
  ...
  if (type == LEFTRIGHT::LEFT) {
    ierr = MatCreateVecs(m, nullptr, &vector);
    CHKERRV(ierr); // a vector with the same number of rows
  }
  else {
    ierr = MatCreateVecs(m, &vector, nullptr);
    CHKERRV(ierr); // a vector with the same number of cols
  }
  setName(vector, name);
}
...
void
Vector::setValue(PetscInt row, PetscScalar value)
{
  PetscErrorCode ierr = 0;
...
void
Vector::arange(double start, double stop)
{
  PetscErrorCode ierr = 0;
  PetscScalar *  a;
...
void
Vector::write(std::string filename, VIEWERFORMAT format)
{
  PetscErrorCode ierr = 0;
  PetscViewer    viewer;
...
void
Vector::read(std::string filename, VIEWERFORMAT format)
{
  PetscErrorCode ierr = 0;
  PetscViewer    viewer;
  openViewer(viewer, filename, format, getCommunicator(vector));
...
void
Matrix::init(PetscInt localRows, PetscInt localCols, PetscInt globalRows, PetscInt globalCols,
...
MatInfo
Matrix::getInfo(MatInfoType flag)
{
  MatInfo info;
...
void
Matrix::setValue(PetscInt row, PetscInt col, PetscScalar value)
{
  PetscErrorCode ierr = 0;
...
void
Matrix::setColumn(Vector &v, int col)
{
  PetscErrorCode     ierr = 0;
...
void
Matrix::write(std::string filename, VIEWERFORMAT format)
{
  PetscErrorCode ierr = 0;
  PetscViewer    viewer;
...
bool
KSPSolver::solve(Vector &b, Vector &x)
{
  PetscErrorCode     ierr = 0;
