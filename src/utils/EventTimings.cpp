...
void
EventData::print(std::ostream &out)
{
  using std::setw;
...
void
EventData::writeCSV(std::ostream &out)
{
...
void
EventData::writeEventLog(std::ostream &out)
{
  using namespace std::chrono;
  auto        now = EventRegistry::instance().getTimestamp();
  std::time_t ts  = system_clock::to_time_t(now);
...
void
EventRegistry::initialize(std::string appName, std::string run)
{
...
void
EventRegistry::signal_handler(int signal)
{
  if (initialized) {
    finalize();
    printAll();
...
Event &
EventRegistry::getStoredEvent(std::string const &name)
{
...
EventRegistry::getTimestamp()
{
  return timestamp;
}

Event::Clock::duration
EventRegistry::getDuration()
{
...
void
EventRegistry::print(std::ostream &out)
{
  ...
  if (rank == 0) {
    ...
    Table table({{getMaxNameWidth(), "Event"},
                 {10, "Count"},
                 {10, "Total[ms]"},
                 {10, "Max[ms]"},
                 {10, "Min[ms]"},
...
void
EventRegistry::writeCSV(std::string filename)
{
  int rank = Parallel::getProcessRank();
  int size = Parallel::getCommunicatorSize();
...
void
EventRegistry::writeEventLogs(std::string filename)
{
  int rank = Parallel::getProcessRank();
...
std::map<std::string, GlobalEventStats>
getGlobalStats(GlobalEvents events)
{
...
void
EventRegistry::printGlobalStats()
{
