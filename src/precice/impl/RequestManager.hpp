...
public:
  ...
  void
  requestGetMeshVertexIDsFromPositions(
      int     meshID,
      int     size,
      double *positions,
      int *   ids);
  ...
  void
  requestSetMeshTriangleWithEdges(
      int meshID,
      int firstVertexID,
      int secondVertexID,
      int thirdVertexID);
  ...
  void
  requestMapWriteDataFrom(int fromMeshID);

  /// Requests read mapping data from server.
  void
  requestMapReadDataTo(int toMeshID);
...
private:
  ...
  void
  handleRequestInitialze(const std::list<int> &clientRanks);

  /// Handles request initialize data from client.
  void
  handleRequestInitialzeData(const std::list<int> &clientRanks);

  /// Handles request advance from client.
  void
  handleRequestAdvance(const std::list<int> &clientRanks);
  ...
  void
  handleRequestFulfilledAction(int rankSender);

  /// Handles request set mesh vertex from client.
  void
  handleRequestSetMeshVertex(int rankSender);

  /// Handles request get mesh vertex size from client.
  void
  handleRequestGetMeshVertexSize(int rankSender);

  /// Handles request reset mesh from client.
  void
  handleRequestResetMesh(int rankSender);

  /// Handles request set vertex positions from client.
  void
  handleRequestSetMeshVertices(int rankSender);

  /// Handles request get vertex positions from client.
  void
  handleRequestGetMeshVertices(int rankSender);

  /// Handles request get vertex IDs from client.
  void
  handleRequestGetMeshVertexIDsFromPositions(int rankSender);

  /// Handles request set mesh edge from client.
  void
  handleRequestSetMeshEdge(int rankSender);

  /// Handles request set mesh triangle from client.
  void
  handleRequestSetMeshTriangle(int rankSender);

  /// Handles request set mesh triangle with edges from client.
  void
  handleRequestSetMeshTriangleWithEdges(int rankSender);

  /// Handles request set mesh quad from client.
  void
  handleRequestSetMeshQuad(int rankSender);

  /// Handles request set mesh quad with edges from client.
  void
  handleRequestSetMeshQuadWithEdges(int rankSender);

  /// Handles request write block scalar data from client.
  void
  handleRequestWriteBlockScalarData(int rankSender);

  /// Handles request write scalar data from client.
  void
  handleRequestWriteScalarData(int rankSender);

  /// Handles request write block vector data from client.
  void
  handleRequestWriteBlockVectorData(int rankSender);

  /// Handles request write vector data from client.
  void
  handleRequestWriteVectorData(int rankSender);

  /// Handles request read block scalar data from client.
  void
  handleRequestReadBlockScalarData(int rankSender);

  /// Handles request read scalar data from client.
  void
  handleRequestReadScalarData(int rankSender);

  /// Handles request read block vector data from client.
  void
  handleRequestReadBlockVectorData(int rankSender);

  /// Handles request read vector data from client.
  void
  handleRequestReadVectorData(int rankSender);

  /// Handles request map written data from client.
  void
  handleRequestMapWriteDataFrom(const std::list<int> &clientRanks);

  /// Handles request map read data from client.
  void
  handleRequestMapReadDataTo(const std::list<int> &clientRanks);
};
