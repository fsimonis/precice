...
public:
  ...
  const DataContext &
  dataContext(int dataID) const;

  DataContext &
  dataContext(int dataID);

  const utils::ptr_vector<DataContext> &
  writeDataContexts() const;

  utils::ptr_vector<DataContext> &
  writeDataContexts();

  const utils::ptr_vector<DataContext> &
  readDataContexts() const;

  utils::ptr_vector<DataContext> &
  readDataContexts();

  bool
  isMeshUsed(int meshID) const;

  bool
  isDataUsed(int dataID) const;

  const MeshContext &
  meshContext(int meshID) const;

  MeshContext &
  meshContext(int meshID);

  const std::vector<MeshContext *> &
  usedMeshContexts() const;

  std::vector<MeshContext *> &
  usedMeshContexts();

  void
  addReadMappingContext(MappingContext *mappingContext);

  void
  addWriteMappingContext(MappingContext *mappingContext);

  const utils::ptr_vector<MappingContext> &
  readMappingContexts() const;

  const utils::ptr_vector<MappingContext> &
  writeMappingContexts() const;

  void
  addWatchPoint(const PtrWatchPoint &watchPoint);
  ...
  void
  addAction(const action::PtrAction &action);
  ...
  void
  setClientServerCommunication(com::PtrCommunication communication);

  com::PtrCommunication
  getClientServerCommunication() const;
...
private:
  ...
  void
  checkDuplicatedUse(const mesh::PtrMesh &mesh);

  void
  checkDuplicatedData(const mesh::PtrData &data);
