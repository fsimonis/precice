...
private:
  ...
  getGeoFilter(const std::string &geoFilter) const;

  mesh::PtrMesh
  copy(const mesh::PtrMesh &mesh) const;
  ...
  mapping::PtrMapping
  getMapping(const std::string &mappingName);

  void
  finishParticipantConfiguration(const impl::PtrParticipant &participant);
};
