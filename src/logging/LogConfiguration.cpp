...
public:
  formatter_type
  create_formatter(boost::log::attribute_name const &name, args_map const &args)
  {
...
public:
  ...
  void
  consume(boost::log::record_view const &rec, string_type const &formatted_record)
  {
    *_ostream << formatted_record << std::endl
              << std::flush;
  }
};
...
readLogConfFile(std::string const &filename)
{
...
void
BackendConfiguration::setOption(std::string key, std::string value)
{
...
void
setupLogging(LoggingConfiguration configs, bool enabled)
{
...
void
setupLogging(std::string const &logConfigFile)
{
  setupLogging(readLogConfFile(logConfigFile));
}

void
setMPIRank(int const rank)
{
