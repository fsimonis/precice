...
struct BackendConfiguration
{
  ...
  void
  setOption(std::string key, std::string value);
};
...
void
setupLogging(std::string const &logConfigFile = "log.conf");

/// Configures the logging from a LoggingConfiguration
void
setupLogging(LoggingConfiguration configs, bool enabled = true);
